...
class PatientController {
  ...
  static async uploadProfileImage(req, res) {
    try {
      ...
      if (patient.profile_image && !patient.profile_image.startsWith('http')) {
        const oldFilename = patient.profile_image.split('/').pop();
        deleteOldImage(oldFilename);
      }

      // Update profile image in users table
      const imageUrl = getImageUrl(req.file.filename);
      await User.update(patient.user_id, { profile_image: imageUrl });

      // Also update in patients table
      await Patient.update(patient.id, { profile_image: imageUrl });
